...
public class Sum3{
  ...
  public Sum3(){
    ...
    CLBuffer<FloatBuffer> BufferResult = cl.createFloatBuffer(tmpfb.limit(),
							 WRITE_ONLY); 
    ...
    kernel.putArgs(BufferA, BufferB, BufferC,BufferResult);
    ...
    queue.putWriteBuffer(BufferA, false)//BufferAのデータをカーネル側へ転送指令
      ...
      .putBarrier() //今までの指令がすべて完了するまで待つ
      .putReadBuffer(BufferResult, true);//BufferCのデータをホスト側へ転送指令
                                    //転送完了まで待つ
    //演算結果の出力
    FloatBuffer fb = BufferResult.getBuffer();
    fb.rewind();
