...
public class CameraJavaCV implements CameraDevice{
  ...
  private final boolean RightProportion;
   ...
   * @param height 画像の高さ
   */
  public CameraJavaCV(int width, int height, boolean rightProportion){
    this(0,width,height, rightProportion);
  }
   ...
   *  @param rightProportion 撮影画像の画素の縦横比を調整処理の有効化
   */
  public CameraJavaCV(int device, int width, int height, boolean rightProportion){
    framegrabber = new OpenCVFrameGrabber(device);
    converter = new Java2DFrameConverter();
    converterimpi = new OpenCVFrameConverter.ToIplImage();
  ...
  public void resizedGrab(BufferedImage img) {
    framegrabber.setImageWidth(WIDTH);
    framegrabber.setImageHeight(HEIGHT);
    try {
      if( WIDTH != framegrabber.getImageWidth() ||
          HEIGHT != framegrabber.getImageHeight()) {
        IplImage grabbed = converterimpi.convert(framegrabber.grab());
        int tmpheight;
        int tmpwidth;
        if(RightProportion){
          float gheight = framegrabber.getImageHeight();
          float gwidth = framegrabber.getImageWidth();
          if(gheight/gwidth > 1.0f){
            tmpheight = HEIGHT;
            tmpwidth = (int)(WIDTH*gwidth/gheight);
          }else{
            tmpheight = (int)(HEIGHT*gheight/gwidth);
            tmpwidth = WIDTH;
          }
        }else{
          tmpheight = HEIGHT;
        ...
        IplImage resized = IplImage.create(tmpwidth, tmpheight, grabbed.depth(), grabbed.nChannels());
        cvResize(grabbed, resized);
        Frame frame = converterimpi.convert(resized);
        Java2DFrameConverter.copy(frame, img);
      } else {
        Java2DFrameConverter.copy(framegrabber.grab(), img);
      }
    }catch(org.bytedeco.javacv.FrameGrabber.Exception e){
