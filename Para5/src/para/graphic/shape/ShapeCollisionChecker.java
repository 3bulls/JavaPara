...
public class ShapeCollisionChecker{
  ...
  public static Shape CollisionCheck(ShapeManager sm, Vec2 p0, Vec2 v,
    ...
    for(int i=0;i<data.length;i++){
      ...
      float hittime;
      // v is the velocity, p0 is the current position of point
      if(MathUtil.crossprod(d0,v)<0){
        // if the direction of v is on the right side of d0
        hittime = isIntersect(v2,v3,p0,v,resttime[0]);
      }else{
        // if the direction of v is on the left side of d0
        hittime = isIntersect(v0,v1,p0,v,resttime[0]);
      }
      if(0<=hittime){
        // will hit the square, under the effection of reflection
        // the velocity of (vx,vy) will be (vx,-vy)
        vnewtmp = new Vec2(v.data[0],-v.data[1]);
      }else{
        // top and bottom will not be intersected
        // now judge the left and right
