...
public class TargetColorFilter2 extends TargetColorFilter{
  ...
  protected ByteBuffer process(ByteBuffer input){
    ...
    IntStream.range(0,Camera.WIDTH*Camera.HEIGHT)
      .forEach(n->{int min=600000;
          ...
          for(int j=0;j<sample.length;j++){
            // classfy all the pixels into SCount3 type
            // every pixels will enter into a group that closest to it
          ...
          // ant ave will make the the average of color
          ave.get(label).add(inimage[n]);}
        );

    //1番目のParallelStream処理
    // here, we have some problems. reason!
    // IntStream.range(0,Camera.WIDTH*Camera.HEIGHT)
    //   .parallel().unordered()
    //   .forEach(n->{int min=600000;
    //       int label=0;
    ...
    //3番目のStream処理
    //same as TargetColorFilter, choose the closest color to substitute the origin pixel
